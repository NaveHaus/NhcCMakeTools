...
PresetsGraph::ApplyContext(const MacroContext& context)
{
  ...
  std::unordered_set<std::string> attemptedFiles;
  for(const auto rootId : m_RootFileIds) {
    attemptedFiles.insert(m_IncludeGraph.GetFilePayload(rootId).FilePath);
  }
  ...
  while(discoveredNew) {
    ...
    m_IncludeGraph.ResolveIncludes(context);

    for(PresetIncludeGraph::NodeId nodeId = 0;
      nodeId < m_IncludeGraph.FileCount(); ++nodeId)
    {
      const auto& payload = m_IncludeGraph.GetFilePayload(nodeId);
      if(attemptedFiles.find(payload.FilePath) == attemptedFiles.end()) {
        discoveredNew = true;
        attemptedFiles.insert(payload.FilePath);
        (void) TryLoadFileNode(nodeId);
      }
    }
  }
